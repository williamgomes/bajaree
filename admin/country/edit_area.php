...
if (isset($_POST['submit'])) {
    ...
    if ($city == "") {
        ...
        $CheckArea = "SELECT * FROM areas WHERE area_id != " . intval($areaid) . " AND area_name='$area' AND area_city_id='$city'";
        $ExecuteCheck = mysqli_query($con, $CheckArea);
        if ($ExecuteCheck) {
            if (mysqli_num_rows($ExecuteCheck) > 0) {
                $err = "(<b>" . $area . "</b>) already added ";
            } else {
                ...
                $AddAreaSql = "UPDATE  areas SET $AddArea WHERE area_id=" . intval($areaid);
            ...
            if (DEBUG) {
                $err = 'ExecuteSql Error: ' . mysqli_error($con);
            } else {
                $err = "City add failed.";
            }
        }
...
/* edit area query */
$areaSql = "SELECT * FROM areas WHERE area_id=" . intval($areaid);
$areaSqlResult = mysqli_query($con, $areaSql);
if ($areaSqlResult) {
    while ($areaSqlResultObj = mysqli_fetch_object($areaSqlResult)) {
        $city = $areaSqlResultObj->area_city_id;
        $area = $areaSqlResultObj->area_name;
    }
} else {
    if (DEBUG) {
                ...
                <div class="widget first">
                    ...
                    <div class="body">
                        <div class="charts" style="width: 700px; height: auto;">
                            <form action="edit_area.php?cid=<?php echo $_GET['cid']; ?>" method="post" class="mainForm">

                                <!-- Input text fields -->
                                <fieldset>
                                    <div class="widget first">
                                        <div class="head">
                                            <h5 class="iList">Edit Area</h5></div>
                                                ...
                                                <select name="city">
                                                    ...
                                                    if ($countCity > 0):
                                                        for ($i = 0; $i < $countCity; $i++):
                                                            ?>
                                                            <option value="<?php echo $arrayCity[$i]->city_id; ?>" <?php
                                                            if ($arrayCity[$i]->city_id == $city) {
                                                                echo "selected";
                                                            }
                                                            ?>><?php echo $arrayCity[$i]->city_name; ?></option>
                                                                    <?php
                                                                endfor;
                                                            endif;
                                                            ?>
                                        ...
                                        <input type="submit" name="submit" value="Edit Area" class="greyishBtn submitForm" />
                                        <div class="fix"></div>
