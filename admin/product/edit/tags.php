...
if ($executeProductTag) {
  ...
  if (isset($executeProductTagObj->product_id) AND $executeProductTagObj->product_tags !="") {
...
if(isset($_GET['delstr'])){
  ...
  if(($key = array_search($delStr, $oldTagStr)) !== false){
    ...
    if($executeUpdateTagSQL){
      redirect('tags.php?pid=' . $_GET['pid']);
    }
    //var_dump($oldTagStr);
  }
}
...
if (isset($_POST['add'])) {
  ...
  if (sizeof($new_tags) == 0) {
    ...
    if(count($oldTags) > 0 AND count($new_tags) > 0){
  ...
  ?>
                            ...
                            for ($i = 0; $i < $countArrayTags; $i++):
                              if (!in_array($arrayTags[$i]->tag_title, $tags)) {
                                ?>     
                                <option value="<?php echo trim($arrayTags[$i]->tag_title); ?>"><?php echo $arrayTags[$i]->tag_title; ?></option>
                                <?php
                              }
                              ?>
