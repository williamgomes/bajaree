...
if (isset($_POST['admin_create']) AND $_POST['admin_create'] == 'Submit') {
    ...
    $admin_type = trim($_POST['admin_type']);
    if (isset($_POST['admin_full_name']) AND $_POST['admin_full_name'] == '') {
        $err = 'Name field is required!!';
    } elseif (!filter_var($admin_email, FILTER_VALIDATE_EMAIL) || !preg_match('!^[\w @.-]*$!', $admin_email)) {
    ...
    } elseif (isset($_POST['admin_password']) AND $_POST['admin_password'] == '') {
        $err = 'Admin password field is required!!';
    } elseif (isset($_POST['admin_password']) AND (strlen($_POST['admin_password']) > $config['ADMIN_PASSWORD_LENGTH_MAX'] OR strlen($_POST['admin_password']) < $config['ADMIN_PASSWORD_LENGTH_MIN'])) {
        $err = 'Password length should be with in maximum: ' . $config['ADMIN_PASSWORD_LENGTH_MAX'] . ' AND Minimum: ' . $config['ADMIN_PASSWORD_LENGTH_MIN'] . '!! Given password length is :' . strlen($_POST['admin_password']);
    } elseif (isset($_POST['admin_conf_password']) AND $_POST['admin_conf_password'] == '') {
        $err = 'Confirm password  filed is required!!';
    } elseif (isset($_POST['admin_conf_password']) AND $_POST['admin_conf_password'] != $_POST['admin_password']) {
                ...
                <div class="widget first">
                    ...
                    <div class="body">
                        <div class="charts" style="width: 700px; height: auto;">
                            <form action="admin_create.php" method="post" class="mainForm">
